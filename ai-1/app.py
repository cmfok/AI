...
# ── 幻灯片主题定义（用于 AI 分类）────────────────────
SLIDES = [
    {"page": 1,  "title": "封面",                     "tags": "封面 开始 青骏会"},
    {"page": 2,  "title": "大家都在问",                 "tags": "热门AI话题 日常AI 手机 抖音 美颜 热议"},
    {"page": 3,  "title": "你熟悉的AI",                 "tags": "企业AI 痛点 不准确 忘了开头 通用回复 改来改去 方向偏差"},
    {"page": 4,  "title": "反认知的事实",               "tags": "AI猜字 猜字系统 不思考 不推理 反认知"},
    {"page": 5,  "title": "AI的先天问题",               "tags": "LLM 大语言模型 预训练 幻觉 数学 知识截止 先天缺陷"},
    {"page": 6,  "title": "靠猜≠不能用",               "tags": "四大工程 转折 可用 提示词工程 上下文工程 规训工程 循环工程"},
    {"page": 7,  "title": "提示词工程",                 "tags": "提示词 Prompt 派活 角色 要求 参考 标准"},
    {"page": 8,  "title": "上下文工程",                 "tags": "上下文 Context 资料 历史数据 参考模板 业务规则 知识库 RAG"},
    {"page": 9,  "title": "规训工程",                   "tags": "规训 Harness 工具 API MCP 脚本 工作流 边界 系统对接 自动化 集成"},
    {"page": 10, "title": "循环工程",                   "tags": "循环 Loop 自查 复盘 会诊 绩效 反馈 PDCA"},
    {"page": 11, "title": "三个通用场景",               "tags": "场景 通用 分类 数据比对 流程监控 内容生成"},
    {"page": 12, "title": "场景一：数据比对",           "tags": "数据比对 对账 核对 差异 采购 运费 库存"},
    {"page": 13, "title": "场景二：流程监控",           "tags": "流程监控 审批 预警 告警 自动化 入职 生产 排产 物料 制造 工厂 流程 脱节 手工"},
    {"page": 14, "title": "场景三：内容生成",           "tags": "内容生成 智能问答 知识库 营销 文案 客服"},
    {"page": 15, "title": "判断标准",                   "tags": "判断 标准 适合 不适合 AI适用 评估"},
    {"page": 16, "title": "AI落地避坑指南",             "tags": "避坑 落地 期望过高 数据脏 没配套 失败"},
    {"page": 17, "title": "从哪开始？三步走",           "tags": "三步走 行动 诊断 试点 推广 路线图"},
    {"page": 18, "title": "使用AI前思考三个问题",       "tags": "判断AI方案 评估 供应商 选型 三个问题"},
    {"page": 19, "title": "听众自由提问",               "tags": "自由提问 开放 其他 杂项 综合 未归类 听众问题"},
]

# 不展示问题的封面/过渡页（不参与 AI 分类，已有问题强制迁移）
COVER_PAGES = {1, 2, 4, 6, 11}
# 无法归类的问题统一放到此页
FALLBACK_PAGE = 19
...
DEEPSEEK_API_KEY = os.environ.get('DEEPSEEK_API_KEY', '')  # DeepSeek API Key (set via env)
...
def control_go():
    ...
    for k in ["expand", "card", "mode", "lock"]:
        if k in data:
            CONTROL_STATE[k] = data[k]
...
@app.route('/ai')
def ai_help_page():
    return app.send_static_file('ai_scroll.html')

@app.route('/ai/1')
def ai_1():
    return app.send_static_file('ai/1/index.html')

@app.route('/ai/1/control')
def ai_1_control():
    return app.send_static_file('ai/1/control.html')

@app.route('/ai/1/listen')
def ai_1_listen():
    return app.send_static_file('ai/1/listen.html')

@app.route('/ai/1/qa')
def ai_1_qa():
    return app.send_static_file('ai/1/qa.html')
...
@app.route('/ai/2/control')
def ai_2_control():
    return app.send_static_file('control.html')

@app.route('/ai/2/listen')
def ai_2_listen():
...
@app.route('/ai/2')
def ai_2():
    return app.send_static_file('ai/2/index.html')

# ── AI 分享会第三版（ai3）─────────────────────────────
@app.route('/ai/3')
def ai_3():
    return app.send_static_file('ai/3/index.html')

@app.route('/ai/3/control')
def ai_3_control():
    return app.send_static_file('ai/3/control.html')

@app.route('/ai/3/listen')
def ai_3_listen():
    return app.send_static_file('ai/3/listen.html')

@app.route('/ai/3/qa')
def ai_3_qa():
    return app.send_static_file('ai/3/qa.html')
